 ...
 * Drop this in your Next.js project at: src/lib/api.ts
 * 
 * In development, Python backend runs at http://localhost:8000
 * In production, set NEXT_PUBLIC_API_URL to your deployed backend URL
 */

const BASE = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000"

async function fetchJSON<T>(url: string, options?: RequestInit): Promise<{ data: T | null; error: string | null; count?: number }> {
  try {
